...
require "getopts.pl";
&Getopts('hdDmM');		      # -h -d -s, set opt_h, opt_d, etc
...
#	    	print "$_ \n";
	...
	if(defined($var_map{"trigger"}))
	{
		$ncl = "";
		$ncl .= $var_map{"trigger"};
		$ncl .= " 0\n";
		$clauses[$cl_idx++]  = $ncl;
		if ($opt_d) {print STDERR "Trigger appended to CNF File\n";}

	} else
	{
		print STDERR "Trigger Variable Undefined\n";
	}


	&prt_clauses;
	if (defined($opt_m)) {
	    &prt_map ($mapf, \%var_map, \@gt_inputs, \@gt_outputs);
	}
	if (defined($opt_M)) {
	    &prt_map ($mapf, \%var_map, \@gt_inputs, \@gt_outputs, true);
	}
	...
	if(m/\.inputs\s+([\w\d\-\_\.\(\)\[\]\s\$]+)/) {
	$input_vars = $1;
	###print "inputs = $input_vars \n";
	if (m/\\/) {    # Contemplate a *single* line extension.
	    do {
		$line = <>;
		$line =~ m/([\w\d\-\_\.\(\)\s]+)/;
	...
	@gt_inputs = split /\s+/, $input_vars;
	print STDERR "@gt_inputs \n";
	if ($opt_D) { &prt_array ("CRKT INPUTS ", \@gt_inputs); }
	}
	if(m/\.outputs\s+([\w\d\-\_\.\(\)\[\]\s\$]+)/) {
	$output_vars = $1;
	###print "outputs = $output_vars \n";
	if (m/\\/) {    # Contemplate a *single* line extension.
	    do {
		$line = <>;
		$line =~ m/([\w\d\-\_\.\(\)\s]+)/;
	...
	@gt_outputs = split /\s+/, $output_vars;
	print STDERR "@gt_outputs \n";
	if ($opt_D) {  &prt_array ("CRKT OUTPUTS ", \@gt_outputs);}
	} else {
...
sub prt_map {
    local ($fname, $vm_ref, $i_ref, $j_ref, $ON) = @_;
    ...
    local *OUTP;
    open (OUTP, ">$fname") || die "Unable to open map file $opt_m";

    @$i_ref = reverse(@$i_ref);
    @$j_ref = reverse(@$j_ref); 	

    $noinputs = scalar @$i_ref;
    if($opt_M) {print OUTP "inputs($noinputs): ";} else {print OUTP "$noinputs ";}

    foreach $var (@$i_ref) {
	print OUTP "$$vm_ref{$var} ";
    }
    print OUTP " 0\n"; 

    $nooutputs = scalar @$j_ref;
     if($opt_M) {print OUTP "outputs($nooutputs): ";} else {print OUTP "$nooutputs ";}
    foreach $var (@$j_ref) {
	print OUTP "$$vm_ref{$var} ";
    }
    print OUTP " 0\n"; 
